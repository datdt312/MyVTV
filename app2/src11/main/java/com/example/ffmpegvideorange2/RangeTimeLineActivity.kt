...
class RangeTimeLineActivity : AppCompatActivity(){
    ...
    private fun initView(){
        ...
        handler.setVelocityTrackerListener(object :VelocityTrackListener{
            override fun onVelocityChanged(velocity: Int) {
                ...
                if (binding.rvFrame.getAvFrameHelper()?.isSeekBack == true && velocity == 0){
                    binding.rvFrame.getAvFrameHelper()?.isScrolling = false
                ...
                if (velocity < 0){
                    if (binding.rvFrame.getAvFrameHelper()?.isScrolling == false){
                        binding.rvFrame.getAvFrameHelper()?.isScrolling = true
                ...
                if (velocity > 0){
                    ...
                    if (binding.rvFrame.getAvFrameHelper()?.isSeekBack == true){
                        binding.rvFrame.getAvFrameHelper()?.isSeekBack = false
                    }
                }else if(velocity < 0){
                    //预览条向后滑动
                    if (binding.rvFrame.getAvFrameHelper()?.isSeekBack == false){
                        binding.rvFrame.getAvFrameHelper()?.isSeekBack = true
                    }
                }
        ...
        /*binding.rvFrame.addOnChildAttachStateChangeListener(object:RecyclerView.OnChildAttachStateChangeListener{
    ...
    fun begin() {
        ...
        kzgPlayer!!.setGetFrameListener(object :KzgPlayer.GetFrameListener{
            ...
            override fun getFramePacket(dataSize: Int, pts: Double, data: ByteArray?) {
                ...
                if (binding.rvFrame.getAvFrameHelper() is IMediaCodecFrameHelper){
                    (binding.rvFrame.getAvFrameHelper() as IMediaCodecFrameHelper).packetQueue.enQueue(packetBean)
                    if ((binding.rvFrame.getAvFrameHelper() as IMediaCodecFrameHelper).packetQueue.queueSize >= 30){
                        binding.rvFrame.getAvFrameHelper()?.pause()
                    }
                }
            ...
            override fun onGetFrameYUV(width: Int, height: Int, y: ByteArray?, u: ByteArray?, v: ByteArray?,
                ...
                runOnUiThread {
                    if (binding.rvFrame.getAvFrameHelper() is IFFmpegCodecFrameHelper){
                        ...
                        bean.u = u
                        bean.v = v
                        Log.e("kzg","********************yuvQueue:${ (binding.rvFrame.getAvFrameHelper() as IFFmpegCodecFrameHelper).yuvQueue.queueSize}")
                        (binding.rvFrame.getAvFrameHelper() as IFFmpegCodecFrameHelper).yuvQueue.enQueue(bean)
                        if ((binding.rvFrame.getAvFrameHelper() as IFFmpegCodecFrameHelper).yuvQueue.queueSize > 2){
                            binding.rvFrame.getAvFrameHelper()?.pause()
                        }
                    }
                }
